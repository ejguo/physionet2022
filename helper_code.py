...
# Load recordings.
# Input: 2530.txt, where 2530 is patient ID
# Algorithm: read Input to get number of locations and their wav filenames, read all the
#            wav files and make a list
# Output: [ recording_0, recording_1, recording_2, recording_3 ] if there are 4 locations
#           recording_i = [ channel_0, channel_1 ] if there are two channels i.e., wav[1].shape[1]
#           channel_j = [ amp_0, amp_1, ..., amp_m] where m = wav[1].shape[0]
#           wav[0] is frequencies, that many data points make a second
...
def get_murmur_locations(data):
    ...
    if locations is None:
        raise ValueError('Bad #Murmur locations:')
    if locations.lower() == 'nan':
        return None
