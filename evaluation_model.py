...
def binary_outputs(scalar_outputs, classes):
    ...
    for i, y in enumerate(scalar_outputs):
        j = np.argmax(y)
        bin[i, j] = True
    return bin
...
def evaluate_model(murmur_labels, murmur_scalar_outputs, outcome_labels, outcome_scalar_outputs):

    # Load and parse label and model output files.

    # For each patient, set the 'Present' or 'Abnormal' class to positive if no class is positive or if multiple classes are positive.
    murmur_labels = enforce_positives(murmur_labels, murmur_classes, 'Present')
    murmur_binary_outputs = binary_outputs(murmur_scalar_outputs, murmur_classes)
    murmur_binary_outputs = enforce_positives(murmur_binary_outputs, murmur_classes, 'Present')
    outcome_labels = enforce_positives(outcome_labels, outcome_classes, 'Abnormal')
    outcome_binary_outputs = binary_outputs(outcome_scalar_outputs, outcome_classes)
    outcome_binary_outputs = enforce_positives(outcome_binary_outputs, outcome_classes, 'Abnormal')

    # Evaluate the murmur model by comparing the labels and model outputs.
    murmur_auroc, murmur_auprc, murmur_auroc_classes, murmur_auprc_classes = compute_auc(murmur_labels,
    ...
    murmur_weighted_accuracy = compute_weighted_accuracy(murmur_labels, murmur_binary_outputs,
                                                         murmur_classes)  # This is the murmur scoring metric.
    murmur_cost = compute_cost(outcome_labels, murmur_binary_outputs, outcome_classes,
                               murmur_classes)  # Use *outcomes* to score *murmurs* for the Challenge cost metric, but this is not the actual murmur scoring metric.
    murmur_scores = (murmur_classes, murmur_auroc, murmur_auprc, murmur_auroc_classes, murmur_auprc_classes,
                     murmur_f_measure, murmur_f_measure_classes, murmur_accuracy, murmur_accuracy_classes,
                     murmur_weighted_accuracy, murmur_cost)

    # Evaluate the outcome model by comparing the labels and model outputs.
    outcome_auroc, outcome_auprc, outcome_auroc_classes, outcome_auprc_classes = compute_auc(outcome_labels,
                                                                                             outcome_scalar_outputs)
    outcome_f_measure, outcome_f_measure_classes = compute_f_measure(outcome_labels, outcome_binary_outputs)
    outcome_accuracy, outcome_accuracy_classes = compute_accuracy(outcome_labels, outcome_binary_outputs)
    outcome_weighted_accuracy = compute_weighted_accuracy(outcome_labels, outcome_binary_outputs, outcome_classes)
    outcome_cost = compute_cost(outcome_labels, outcome_binary_outputs, outcome_classes,
                                outcome_classes)  # This is the clinical outcomes scoring metric.
    outcome_scores = (outcome_classes, outcome_auroc, outcome_auprc, outcome_auroc_classes, outcome_auprc_classes,
                      outcome_f_measure, outcome_f_measure_classes, outcome_accuracy, outcome_accuracy_classes,
                      outcome_weighted_accuracy, outcome_cost)
...
def find_challenge_files(label_folder, output_folder):
    ...
    for label_file in sorted(os.listdir(label_folder)):
        ...
        if os.path.isfile(label_file_path) and label_file.lower().endswith(
                '.txt') and not label_file.lower().startswith('.'):
...
def print_evaluation(murmur_labels, murmur_scalar_outputs, outcome_labels, outcome_scalar_outputs):
    murmur_scores, outcome_scores = evaluate_model(murmur_labels, murmur_scalar_outputs, outcome_labels,
                                                   outcome_scalar_outputs)
...
# Given per wave test results, return per patient results
# Algorithm:
#    1. divide ids into groups, each group with the same id
#    2. take the average (murmur_probs, outcome_probs)
def print_patient_evaluation(data_dir, ids, murmur_probs, outcome_probs, verbose):
    ...
    patient_murmur_probs = np.zeros((num_patients, num_murmurs), dtype=float)
    patient_outcome_probs = np.zeros((num_patients, num_outcomes), dtype=float)
    patient_murmur_labels = np.zeros((num_patients, num_murmurs), dtype=float)
    patient_outcome_labels = np.zeros((num_patients, num_outcomes), dtype=float)
    ...
    for i in range(num_patients):
        ...
        patient_murmur_probs[i] = np.mean(murmur_probs[start:end, :], axis=0)
        patient_outcome_probs[i] = np.mean(outcome_probs[start:end, :], axis=0)

        file = os.path.join(data_dir, f"{ids[i, 0]}.txt")
        data = load_patient_data(file)

        murmur_label = get_murmur(data)
        j = murmur_classes.index(murmur_label)
        patient_murmur_labels[i, j] = 1

        outcome_label = get_outcome(data)
    ...
    print_evaluation(patient_murmur_labels, patient_murmur_probs, patient_outcome_labels, patient_outcome_probs)

import train_murmur3
import train_outcome
...
if __name__ == "__main__":
    ...
    model = train_murmur3.MurmurModel1().to(device)
    ...
    murmur_labels = np.zeros((n, 3), dtype=int)
    murmur_scalar_outputs = np.zeros((n, 3), dtype=float)
    ...
    for i in test_indices[:n]:
        ...
        with torch.no_grad():
            ...
            murmur_scalar_outputs[j] = y1[0]
            murmur = np.argmax(y1[0])
            murmur_labels[j, murmur] = 1
    ...
    model = train_outcome.MurmurModel1().to(device)
    filename = "outcome.model"
    state_dict = torch.load(filename)
    model.load_state_dict(state_dict)
    ...
    hop_length = 512  # max_len = 152080, try to get 300 time banks, hop_length = 507 or 508
    n_mels = 3
    x, y = get_mfcc_outcome(data_dir, n_fft, hop_length, n_mels, verbose)
    ...
    outcome_labels = np.zeros((n, 2), dtype=int)
    outcome_scalar_outputs = np.zeros((n, 2), dtype=float)
    # get a sample from the dataset for inference
    test_indices = np.random.permutation(len(dataset))
    n = 100
    correct = 0
    ...
    for i in test_indices[:n]:
        ...
        with torch.no_grad():
            y1 = model(x.unsqueeze_(0))
            outcome_scalar_outputs[j] = y1[0]
            outcome = np.argmax(y1[0])
            outcome_labels[j, outcome] = 1
            outcome_true = np.argmax(y)
            message = f"{y1[0]} \t {y} \t "
            if outcome == outcome_true:
                message += f"{outcome}"
                correct += 1
            else:
                message += f"{outcome} <> {outcome_true}"
            print(message)
        j += 1

    print_evaluation(murmur_labels, murmur_scalar_outputs, outcome_labels, outcome_scalar_outputs)